...
class CacheStore:
    ...
    async def preload_all(self, user, config_repo, config):
        ...
        # Fetch summary files (2 requests instead of N)
        pdb_summary = await github_raw_json(user, config_repo, "cache/pdb/summary.json")
        bgpq4_summary = await github_raw_json(user, config_repo, "cache/bgpq4/summary.json")
        ...
        for asn in sorted(all_asns):
            ...
            if asn_type == 0:
                ...
                self.pdb[asn] = pdb_summary[str(asn)]
        ...
        for asn in sorted(peer_downstream_asns):
            if validateASN(asn) == 1 and bgpq4_summary and str(asn) in bgpq4_summary:
                self.bgpq4[asn] = bgpq4_summary[str(asn)]
...
def _build_index_html(config, local_asn, base):
    """Build HTML index page showing full vyos.yaml info."""

    css = ("body{font-family:monospace;max-width:960px;margin:40px auto;padding:0 20px;"
    ...
    p.append(f'<p>Member limit: <code>{asl.get("member-limit", "N/A")}</code> &nbsp; '
             f'Prefix limit: <code>{asl.get("prefix-limit", "N/A")}</code></p>')
    lal = asl.get("large-as-list", [])
    if lal:
        p.append('<p>Large AS list: ' + ", ".join(f"<code>AS{a}</code>" for a in lal) + '</p>')
    p.append('</div>')
    ...
    if keepup:
        ...
        p.append('<p>' + ", ".join(f"<code>AS{a}</code>" for a in keepup) + '</p></div>')
    ...
    if bl:
        ...
        bl_asns = bl.get("asn", [])
        if bl_asns:
            p.append('<p><strong>ASN:</strong> ' + ", ".join(f"<code>AS{a}</code>" for a in bl_asns) + '</p>')
        bl_asset = bl.get("as-set", [])
        if bl_asset:
            p.append('<p><strong>AS-Set:</strong> ' + ", ".join(f"<code>{a}</code>" for a in bl_asset) + '</p>')
        bl_p4 = bl.get("prefix4", [])
        if bl_p4:
            p.append('<p><strong>IPv4:</strong> ' + ", ".join(f"<code>{px}</code>" for px in bl_p4) + '</p>')
        bl_p6 = bl.get("prefix6", [])
        if bl_p6:
            p.append('<p><strong>IPv6:</strong> ' + ", ".join(f"<code>{px}</code>" for px in bl_p6) + '</p>')
        p.append('</div>')
    ...
    for router in config.get("router", []):
        rn = router["name"]
        script_url = f"{base}/router/configure.{rn}.sh"
        p.append('<div class="section">')
        p.append(f'<h3>{rn} &nbsp; <a href="{script_url}">\u2b07 configure.{rn}.sh</a></h3>')
        ...
        for ntype in ["upstream", "routeserver", "peer", "downstream", "ibgp"]:
            ...
            if not neighbors:
                continue
            p.append(f'<p><span class="badge {ntype}">{ntype}</span> ({len(neighbors)} sessions)</p>')
            p.append('<table><tr><th>ASN</th><th>Neighbor Address</th><th>Interface</th></tr>')
            for nb in neighbors:
                asn_val = nb.get("asn", "?")
                ...
                p.append(f'<tr><td>AS{asn_val}</td><td>{addr_str}</td><td>{iface}</td></tr>')
            p.append('</table>')

        rpki = router.get("protocols", {}).get("rpki", [])
        if rpki:
            rpki_str = ", ".join(f'{r.get("server","?")}:{r.get("port","?")}' for r in rpki)
    ...
    p.append(f'<li><a href="{base}/router/defaultconfig.sh">defaultconfig.sh</a></li>')
    p.append(f'<li><a href="{base}/find_unused.py">find_unused.py</a></li>')
...
async def _handle(request):
    ...
    # Route: / (index) — only needs vyos.yaml
        ...
        html = _build_index_html(config, local_asn, base)
