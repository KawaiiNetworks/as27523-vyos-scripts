...
def save_bgpq4_cache(config_dir, peer_downstream_asns, fill_missing):
    ...
    # 先顺序筛选出需要查询的 ASN（跳过逻辑与计数保持简单、确定）
    tasks = []  # list of (asn, path, as_set_list)
    for asn in peer_downstream_asns:
        ...
        tasks.append((asn, path, as_set_list))
    ...
    with ThreadPoolExecutor(max_workers=BGPQ4_CONCURRENCY) as executor:
        future_map = {executor.submit(run_one, t): t[0] for t in tasks}
